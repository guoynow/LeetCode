...
int kruskal(){
	...
	int res = 0, cnt =0 ;                    // res 记录最小生成树边权之和，cnt 记录当前最小生成树种的边数 
	for(int i = 0; i < m; i ++ ) {
		int a = edges[i].a, b = edges[i].b, w = edges[i].w;
		if (find(a) != find(b) ) {           // 最小边权的起点和终点 a，b 不在一个连通块则合并他们 
			p[find(a)] = find(b);
			res += w;
			cnt ++;
		}
	}
	if(cnt < n - 1) return 0x3f3f3f3f;      // n 个点，最小生成树的边应为 n-1 条，少于 n-1 说明没有最小生成树 
	else return res;
}
```
...
int x = stoi(s);    // x = 123465：将字符串转化成 int 类型，传入 string 类型字符串
...
ss = s.substr(1, 2);   // ss = "23"，s = "123456"：截取 s[1] 开始的长度为 2 的子串，返回子串

p = s.find("234");      // p = 1：查找字符第一次出现的位置，如果没有出现过，则返回 string::npos
```
...
vector<int> a(100, 666); // 初始化 100 个 666 
vector<int> b = vector<int> (a.begin(), a.begin() + 3); // 使用数组 a 初始化数组 b 
vector<int> a{1, 2, 3}; // 初始化为 [1, 2, 3]
vector<int> a = {1, 2, 3};
auto v = vector<int>{1, 2, 3};

vector<vector<int>> v(3, vector<int>(4, 666)); // 初始化三行四列，初始值为666
...
a.push_back(x);               // 在尾部添加 x
a.insert(a.begin() + 2, x);   // 在索引 2 的位置插入 x
a.insert(a.end(), b.begin(), b.end()); // 将数组 b 合并至数组 a 的末尾
    
a.pop_back();                       // 删除最后一个元素
a.erase(a.begin(), a.begin() + 3);  // 删除[索引 0, 索引 2]之间的元素
```
...
```c++
#include <set>    // set 去重， multiset 不去重，均默认升序排序，底层红黑树
...
s.insert(x);  // 在集合中插入 x
s.find(x);    // 查找 x，如果找到则返回x第一次出现位置的迭代器，否则返回尾迭代
s.count(x);    // 返回 x 的个数
s.erase(x);    // 删除 x
```
...
```c++
#include <map>    // map 去重，multimap 不去重，均默认以 key 值(第一属性)升序排序，底层红黑树

map<string, int> m;

m["abc"] ++;  // m["abc"] = 1：插入 "abc"，若不存在则创建，默认值为 0，若存在，访问其值
m["abc"] ++;  // m["abc"] = 2
m["abc"] ++;  // m["abc"] = 3
...
```c++
#include <unordered_set>  // 底层哈希, 操作与 set、map 基本一致
...
struct Student{
	string name;
	double score;
    bool operator < (const node &a) const { // 重载 < 运算符，必须加 const，会默认按重载的小于号逻辑进行排序
        return x > a.x;  // 降序
    }
}stu[N];
...
reverse()   // 传参和 sort 一致，反转区间内的元素顺序
unqiue()    // 传参和和 sort 一致，去重相邻的相同元素，若原序列无序首先需排序，返回去重后原序列尾迭代
```
